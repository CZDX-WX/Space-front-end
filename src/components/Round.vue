<template>
    <main>
        <div data-theme="dark">
            <div class="selector" :class="{ active: isActivated }" :style="selectorStyles">
                <div class="knob" @click="handleClick()"></div>
                <ul>
                    <li v-for="item in menuItems" :key="item.id">
                        <span>{{ item.label }}</span>
                        <input type=radio name=choice :checked="item.checked" />
                    </li>
                </ul>
            </div>
        </div>
...
// import 'https://codepen.io/LukyVj/pen/JjavKEX/ca2344eb8511404c494dd71d1ca8f1a7.js'
import { computed, reactive, ref } from 'vue';

const menuItems = reactive([
    { id: 1, label: 'Back', checked: true },
    { id: 2, label: 'Home', checked: false },
    { id: 3, label: 'Search', checked: false },
    { id: 4, label: 'Star', checked: false },
    { id: 5, label: 'More', checked: false },
    { id: 6, label: 'Refresh', checked: false },
])
...
.selector {
    ...
    &.active {
        ...
        & ul {
            ...
            & li {
                ...
                @for $i from 0 to 7 {
                    &:nth-child(#{$i}) {
                        transform: rotate(#{calc($i * 360 / 6) - 60}deg);
    ...
    & ul {
        ...
        & li {
            ...
            &:before {
                --op: opacity(var(--is-selected)); // need to use a var or the linter yells 
                width: 100%;
                aspect-ratio: 1/1;
            ...
            &:after {
                --iv: invert(var(--invert)); // need to use a var or the linter yells 
                width: 100%;
                aspect-ratio: 1/1;
                pointer-events: none;
            ...
            // Sorry, transform is busy 😅
            &:after,
            &>input[type="radio"] {
                top: calc(var(--icon-offset) * 1%);
            }

            // Didn't found a better way 🤷‍♂️
            &:nth-child(2):after {
